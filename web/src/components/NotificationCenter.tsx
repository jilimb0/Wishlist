    ...
    <div className="absolute top-full right-0 mt-2 w-80 sm:w-96 bg-zinc-900/95 backdrop-blur-xl border border-zinc-800 rounded-2xl shadow-2xl ring-1 ring-white/10 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
      <div className="p-4 border-b border-zinc-800 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h3 className="font-bold text-sm tracking-tight">{t("notifications.title")}</h3>
          {unreadCount > 0 && (
      ...
      <div className="max-h-[70vh] overflow-y-auto scrollbar-hide">
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-xs text-zinc-500 font-medium">{t("notifications.empty")}</p>
          </div>
        ) : (
          <div className="divide-y divide-zinc-800/50">
            {notifications.map((n) => (
              ...
              >
                {!n.isRead && <div className="absolute left-0 top-0 bottom-0 w-1 bg-brand-500" />}
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-zinc-200 mb-0.5">{n.title}</p>
                    <p className="text-xs text-zinc-400 line-clamp-2 mb-1.5 leading-relaxed">
                      {n.message}
                    </p>
                    <span className="text-[10px] text-zinc-600 font-medium">
                      {DateTime.fromISO(n.createdAt).setLocale(language).toRelative()}
                    </span>
                  </div>
                </div>
              </button>
      ...
      {notifications.length > 0 && (
        <div className="p-3 bg-zinc-900/50 border-t border-zinc-800 text-center">
          <button
            type="button"
            className="text-[10px] uppercase tracking-[0.2em] font-black text-zinc-500 hover:text-white transition-colors"
          >
